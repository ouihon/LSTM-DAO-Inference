...
def load_model_C(weights_path=None, norm_params_path=None):
    """
    Load Model C weights and normalization parameters
    ...
    Args:
        weights_path: str, optional - Path to model weights file (.sd format)
                     Defaults to weights/C.sd relative to project root
        norm_params_path: str, optional - Path to normalization parameters JSON file
                         Defaults to weights/Normalization_Params_C.json
    """
    global _model_C, _norm_params_C
    
    # 默认路径
    if weights_path is None:
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        weights_path = os.path.join(base_dir, 'weights', 'C.sd')
    
    if norm_params_path is None:
        # 尝试查找归一化参数文件
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        norm_params_path = os.path.join(base_dir, 'weights', 'Normalization_Params_C.json')
    ...
    # 创建模型
    _model_C = Hybrid_PI_Model(lstm_size=40, mlp_size=16, num_layers=1, K=4).to(_device)
    
    # 加载权重
    if os.path.exists(weights_path):
        checkpoint = torch.load(weights_path, map_location=_device)
        if 'model_state' in checkpoint:
            _model_C.load_state_dict(checkpoint['model_state'])
        else:
            _model_C.load_state_dict(checkpoint)
        _model_C.eval()
        print(f"Model C loaded from {weights_path}")
    else:
        print(f"Warning: Weights file not found at {weights_path}")
        print("Model initialized with random weights")


def predict_C(B, H, T, split_point=500, real_time=None):
    """
    Predict magnetic field strength H using Model C
    ...
    """
    global _model_C, _norm_params_C, _device
    
    if _model_C is None:
        load_model_C()
    
    if _model_C is None:
        raise ValueError("Model C failed to load. Please check the weights file.")
    
    if _norm_params_C is None:
    ...
    # 归一化
    mean_B = _norm_params_C["mean_B"]
    std_B = _norm_params_C["std_B"]
    mean_dB = _norm_params_C["mean_dB"]
    std_dB = _norm_params_C["std_dB"]
    mean_ddB = _norm_params_C["mean_ddB"]
    std_ddB = _norm_params_C["std_ddB"]
    mean_H = _norm_params_C["mean_H"]
    std_H = _norm_params_C["std_H"]
    mean_T = _norm_params_C["mean_T"]
    std_T = _norm_params_C["std_T"]
